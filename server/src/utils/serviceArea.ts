...
export interface PublicMapLocation extends ServiceAreaCoordinates {
  precision: 'approximate';
}

interface TechnicianMapSource {
  id: string;
  location: string;
...
const KNOWN_SERVICE_AREAS: KnownServiceArea[] = [
 ...
 */
export function normalizeServiceAreaInput(value: unknown): ServiceAreaCoordinates | null {
...
function spreadCentroid(area: KnownServiceArea, technicianId: string): ServiceAreaCoordinates {
...
export function getKnownServiceArea(location: string, technicianId: string): ServiceAreaCoordinates | null {
  const normalizedLocation = normalizeLocationLabel(location);
  if (!normalizedLocation) return null;

  const area = KNOWN_SERVICE_AREAS.find(({ aliases }) =>
    aliases.some((alias) => normalizedLocation.includes(alias))
  );
  return area ? spreadCentroid(area, technicianId) : null;
}

export function toPublicMapLocation(technician: TechnicianMapSource): PublicMapLocation | null {
  if (technician.mapVisible === false) return null;
  ...
  return fallback ? { ...fallback, precision: 'approximate' } : null;
}
